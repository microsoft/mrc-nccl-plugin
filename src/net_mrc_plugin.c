...
NCCL_PARAM(MrcRetryCntLinear, "MRC_RETRY_CNT_LINEAR", 7);
NCCL_PARAM(MrcRetryCntExp, "MRC_RETRY_CNT_EXP", 25);
...
typedef struct ncclMRCConnectionMetadata {
...
typedef struct ncclIbNetCommBase {
  ...
  struct mrc_qp_hint *qpHints[NCCL_IB_MAX_QPS];
  int nqps;
  int qpIndex;
  int devIndex;
  struct ncclSocket sock;
...
ncclResult_t ncclIbDestroyBase(struct ncclIbNetCommDevBase* base) {
  ...
  // Also destroy the ibverbs CQ — required before ibv_dealloc_pd
  if (base->cq != NULL) {
...
ncclResult_t ncclMRCCreateQP(uint8_t ib_port, struct ncclIbNetCommDevBase* base, int access_flags, void* qp_context, struct mrc_qp_hint* qpHint, struct ncclIbQp* qp) {
  struct ncclIbDev* ibDev = ncclIbDevs + base->ibDevN;
  struct mrc_context* mrcCtx = ibDev->mrcContext;
  if (mrcCtx == NULL) {
    WARN("NET/IB : MRC Context is NULL for device %s", ibDev->devName);
    return ncclSystemError;
  }
  ...
  NCCLCHECK(wrap_mrc_create_qp(&qp->mrcQp, mrcCtx, &mrcQpInitAttr));
  ...
  qpAttrMask = IBV_QP_STATE | IBV_QP_PKEY_INDEX | IBV_QP_PORT | IBV_QP_ACCESS_FLAGS;

  struct mrc_qp_attr mrcAttr;
  memset(&mrcAttr, 0, sizeof(struct mrc_qp_attr));
  int mrcAttrMask = 0;
  if (qpHint != NULL) {
    mrcAttr.qp_hint = qpHint;
    mrcAttrMask = MRC_QP_HINT;
  }

  NCCLCHECK(wrap_mrc_modify_qp(qp->mrcQp, &qpAttr, qpAttrMask, &mrcAttr, mrcAttrMask));
  TRACE(NCCL_NET, "NET/IB : ncclMRCCreateQP port=%d dev=%d devName=%s ndevs=%d nmdevs=%d pd=%p",
    ib_port, base->ibDevN, ncclIbDevs[base->ibDevN].devName, ncclNIbDevs, ncclNMergedIbDevs, base->pd);
  return ncclSuccess;
}

ncclResult_t ncclMRCRtrQp(struct mrc_qp* mrcQp, struct ncclIbGidInfo* sGidInfo, uint32_t dest_qp_num, struct ncclIbDevInfo* info, bool fifoTc, int tc, int sl) {
  struct ibv_qp_attr qpAttr;
  int same_subnet;
  memset(&qpAttr, 0, sizeof(struct ibv_qp_attr));
  ...
  memset(&mrcAttr, 0, sizeof(struct mrc_qp_attr));
  int mrcAttrMask = 0;
  NCCLCHECK(wrap_mrc_modify_qp(mrcQp, &qpAttr, qpAttrMask, &mrcAttr, mrcAttrMask));
  return ncclSuccess;
}

ncclResult_t ncclMRCRtsQp(struct mrc_qp* mrcQp) {
  struct ibv_qp_attr qpAttr;
  memset(&qpAttr, 0, sizeof(struct ibv_qp_attr));
  ...
  qpAttr.rnr_retry = 7;
  int qpAttrMask = 0;
  qpAttrMask = IBV_QP_STATE | IBV_QP_SQ_PSN | IBV_QP_TIMEOUT | IBV_QP_RETRY_CNT | IBV_QP_RNR_RETRY;
  struct mrc_qp_attr mrcAttr;
  memset(&mrcAttr, 0, sizeof(struct mrc_qp_attr));
  mrcAttr.timeout = ncclParamMrcTimeout();
  // TODO: Add mrc retry cnt attributes when supported by the vendor
  // Comment out for now, as current /opt/mellanox/doca/include/mrc.h does not support retry cnt
  //mrcAttr.retry_cnt.linear_cnt = ncclParamMrcRetryCntLinear();
  //mrcAttr.retry_cnt.exp_cnt = ncclParamMrcRetryCntExp();
  int mrcAttrMask = MRC_QP_TIMEOUT;
  NCCLCHECK(wrap_mrc_modify_qp(mrcQp, &qpAttr, qpAttrMask, &mrcAttr, mrcAttrMask));
  return ncclSuccess;
}
...
ncclResult_t ncclIbCreateFlushQp(uint8_t ib_port, struct ncclIbNetCommDevBase* base, int access_flags, void* qp_context, struct ncclIbQp* qp) {
  ...
  TRACE(NCCL_NET, "NET/IB : ncclIbCreateQp port=%d dev=%d devName=%s ndevs=%d nmdevs=%d qpn=%u pd=%p",
    ib_port, base->ibDevN, ncclIbDevs[base->ibDevN].devName, ncclNIbDevs, ncclNMergedIbDevs, qp->qp->qp_num, base->pd);
  return ncclSuccess;
}
...
ncclResult_t ncclIbGetRequest(struct ncclIbNetCommBase* base, struct ncclIbRequest** req) {
  for (int i=0; i<MAX_REQUESTS; i++) {
    ...
    if (r->type == NCCL_NET_IB_REQ_UNUSED) {
      r->base = base;
      r->sock = NULL;
      for (int j = 0; j < NCCL_IB_MAX_DEVS_PER_NIC; j++) {
        r->devBases[j] = NULL;
        r->events[j] = 0;
      }
      *req = r;
...
static ncclResult_t ncclMRCInitDevices(ncclDebugLogger_t logFunction, ncclProfilerCallback_t profFunction) {
  if (ibRefCount++) return ncclSuccess;
  if (ncclParamIbDisable()) return ncclInternalError;

  // The SendFifo needs to be 32-byte aligned and each element needs
  // to be a 32-byte multiple, so that an entry does not get split and
  // written out of order. This guards against CPU-side torn reads of
  // the FIFO signaling mechanism.
...
static ncclResult_t ncclMRCCreateQpHint(struct ncclIbNetCommBase* commBase, int qpIdx) {
  ...
  struct mrc_context* mrcCtx = ibDev->mrcContext;
  if (mrcCtx == NULL) {
    WARN("NET/IB : mrcContext is NULL for dev %d when creating QP hint for qpIdx %d", devIndex, qpIdx);
    commBase->qpHints[qpIdx] = NULL;
    return ncclSystemError;
  }

  struct mrc_qp_hint_init_attr hintInit;
  memset(&hintInit, 0, sizeof(hintInit));
  hintInit.attr.num_qps_per_peer = commBase->nqps;
  hintInit.attr.num_send_peers = 1;
  INFO(NCCL_NET, "NET/IB : num_qps_per_peer=%d", hintInit.attr.num_qps_per_peer);

  uint32_t ccInitRate = ncclParamMrcCcInitRate();
  uint32_t ccMinRate  = ncclParamMrcCcMinRate() / commBase->nqps;
  uint32_t ccMaxRate  = ncclParamMrcCcMaxRate();
  INFO(NCCL_NET, "NET/IB : ccInitRate=%d ccMinRate=%d ccMaxRate=%d", ccInitRate, ccMinRate, ccMaxRate);
  if (ccInitRate != 0 || ccMinRate != 0 || ccMaxRate != 0) {
    struct mrc_cc_hint ccHint;
    memset(&ccHint, 0, sizeof(ccHint));
    ccHint.version = 1;
    ccHint.init_rate = ccInitRate;
    ccHint.min_rate  = ccMinRate;
    ccHint.max_rate  = ccMaxRate;
    memcpy(hintInit.attr.vendor_cfg, &ccHint, sizeof(struct mrc_cc_hint));
  }


  struct mrc_qp_hint* hint = mrc_create_qp_hint(mrcCtx, &hintInit);
  if (hint == NULL) {
    WARN("NET/IB : mrc_create_qp_hint failed for dev %d qpIdx %d", devIndex, qpIdx);
    commBase->qpHints[qpIdx] = NULL;
    return ncclSystemError;
  }

  commBase->qpHints[qpIdx] = hint;
  return ncclSuccess;
}

ncclResult_t ncclMRCInitCommDevBase(int ibDevN, struct ncclIbNetCommDevBase* base, void* cq_context) {
  base->ibDevN = ibDevN;
  ...
  NCCLCHECK(wrap_mrc_create_cq(&base->mrcCq, ibDev->mrcContext, 2*MAX_REQUESTS*ncclParamIbQpsPerConn(), cq_context, NULL, 0));
...
ncclResult_t ncclMRCInit(void** ctx, uint64_t commId, ncclNetCommConfig_t* config, ncclDebugLogger_t logFunction, ncclProfilerCallback_t profFunction) {
  ncclNetCommConfig_t* netCommConfig = NULL;
  NCCLCHECK(ncclMRCInitDevices(logFunction, profFunction));

  if (ctx) {
    netCommConfig = calloc(1, sizeof(ncclNetCommConfig_t));
...
ncclResult_t ncclMRCDevices(int* ndev) {
  *ndev = ncclNMergedIbDevs;
  return ncclSuccess;
}

ncclResult_t ncclMRCGetProperties(int dev, ncclNetProperties_t* props)
{
  return nccl_p2p_ib_get_properties(ncclIbDevs, ncclNMergedIbDevs, dev, props);
}

ncclResult_t ncclMRCListen(void *ctx, int dev, void* opaqueHandle, void** listenComm) {
  ncclResult_t ret = ncclSuccess;
  struct ncclMRCListenComm* comm;
  comm = calloc(1, sizeof(struct ncclMRCListenComm));
  if (!comm) return ncclInternalError;
...
ncclResult_t ncclMRCCloseSend(void* sendComm);
ncclResult_t ncclMRCCloseRecv(void* recvComm);

ncclResult_t ncclMRCConnect(void *ctx, int dev,  void* opaqueHandle, void** sendComm, ncclNetDeviceHandle_t** sendDevComm) {
  ncclResult_t ret = ncclSuccess;
  struct ncclIbHandle* handle = (struct ncclIbHandle*) opaqueHandle;
  struct ncclIbCommStage* stage = &handle->stage;
  struct ncclIbSendComm* comm = (struct ncclIbSendComm*)stage->comm;
  struct ncclMRCConnectionMetadata remMeta;
  int ready;
...
ib_connect_check:
  ...
  stage->offset = 0;
  struct ncclMRCConnectionMetadata meta;
...
ib_recv_dev_list:
  ...
  for (int i = 0; i < comm->base.vProps.ndevs; i++) {
    ...
    NCCLCHECKGOTO(ncclMRCInitCommDevBase(ibDevN, &comm->devs[i].base, &comm->base.stats), ret, fail);
  }
  ...
  for (int q = 0; q < comm->base.nqps; q++) {
    ...
    // Create QP hint for this QP slot, before creating the QP
    // devIndex must be assigned first
    NCCLCHECKGOTO(ncclMRCCreateQpHint(&comm->base, q), ret, fail);

    NCCLCHECKGOTO(ncclMRCCreateQP(ibDev->portNum, &commDev->base, IBV_ACCESS_REMOTE_WRITE, &comm->base.stats, comm->base.qpHints[q], comm->base.qps + q), ret, fail);
    NCCLCHECKGOTO(mrc_get_qpn(comm->base.qps[q].mrcQp, &meta.qpInfo[q].qpn), ret, fail);
  ...
  for (int i = 0; i < comm->base.vProps.ndevs; i++) {
    ...
    for (int q = 0; q < comm->base.nqps; q++) {
      ...
      if (comm->base.qps[q].devIndex == i)
        ...
        commDev->base.ibDevN, ibDev->portNum, meta.qpInfo[q].qpn, devInfo->mtu, (int64_t)commDev->base.gidInfo.localGidIndex,
...
ib_connect:
  NCCLCHECKGOTO(ncclSocketProgress(NCCL_SOCKET_RECV, &comm->base.sock, stage->buffer, sizeof(ncclMRCConnectionMetadata), &stage->offset, NULL), ret, fail);
  if (stage->offset != sizeof(remMeta)) return ncclSuccess;

  memcpy(&remMeta, stage->buffer, sizeof(ncclMRCConnectionMetadata));
  ...
  for (int q = 0; q < comm->base.nqps; q++) {
    ...
    struct mrc_qp* mrcQp = comm->base.qps[q].mrcQp;

    ncclIbDev* ibDev = ncclIbDevs + commDev->base.ibDevN;
    remDevInfo->mtu = MIN(remDevInfo->mtu, ibDev->portAttr.active_mtu);
    NCCLCHECKGOTO(ncclMRCRtrQp(mrcQp, &commDev->base.gidInfo, remQpInfo->qpn, remDevInfo, false, remMeta.tc, remMeta.sl), ret, fail);
    NCCLCHECKGOTO(ncclMRCRtsQp(mrcQp), ret, fail);
  }
  ...
  if (link_layer == IBV_LINK_LAYER_ETHERNET ) { // RoCE
    for (int q = 0; q < comm->base.nqps; q++) {
      ...
      INFO(NCCL_NET,"NET/IB: IbDev %d Port %d qpn %d",
...
fail:
  if (comm)
    ret = ncclMRCCloseSend(comm);
  goto exit;
}

ncclResult_t ncclMRCAccept(void* listenComm, void** recvComm,  ncclNetDeviceHandle_t** recvDevComm) {
  ncclResult_t ret = ncclSuccess;
  struct ncclMRCListenComm* lComm = (struct ncclMRCListenComm*)listenComm;
...
ib_recv:
  ...
  memcpy(&remMeta, stage->buffer, sizeof(struct ncclMRCConnectionMetadata));
  ...
  for (int i = 0; i < rComm->base.vProps.ndevs; i++) {
    ...
    NCCLCHECKGOTO(ncclMRCInitCommDevBase(ibDevN, &rCommDev->base, &rComm->base.stats), ret, fail);
  ...
  for (int q = 0; q < rComm->base.nqps; q++) {
    ...
    // Create QP hint for this slot — devIndex must be assigned first
    NCCLCHECKGOTO(ncclMRCCreateQpHint(&rComm->base, q), ret, fail);

    NCCLCHECKGOTO(ncclMRCCreateQP(ibDev->portNum, &rCommDev->base, IBV_ACCESS_REMOTE_WRITE, &rComm->base.stats, rComm->base.qpHints[q], qp), ret, fail);
    devIndex = (devIndex + 1) % rComm->base.vProps.ndevs;

    NCCLCHECKGOTO(ncclMRCRtrQp(qp->mrcQp, &rCommDev->base.gidInfo, remMeta.qpInfo[q].qpn, remDevInfo, true, remMeta.tc, remMeta.sl), ret, fail);
    NCCLCHECKGOTO(ncclMRCRtsQp(qp->mrcQp), ret, fail);
  }
  ...
  for (int q = 0; q < rComm->base.nqps; q++) {
    NCCLCHECKGOTO(mrc_get_qpn(rComm->base.qps[q].mrcQp, &meta.qpInfo[q].qpn), ret, fail);
  ...
  NCCLCHECKGOTO(ncclIbMalloc((void**)&stage->buffer, sizeof(struct ncclMRCConnectionMetadata)), ret, fail);
  memcpy(stage->buffer, &meta, sizeof(struct ncclMRCConnectionMetadata));
ib_send:
  NCCLCHECKGOTO(ncclSocketProgress(NCCL_SOCKET_SEND, &rComm->base.sock, stage->buffer, sizeof(struct ncclMRCConnectionMetadata), &stage->offset, NULL), ret, fail);
  if ((size_t)stage->offset < sizeof(struct ncclMRCConnectionMetadata)) return ncclSuccess;
...
fail:
  if (rComm)
    ret = ncclMRCCloseRecv(rComm);
  goto exit;
}

ncclResult_t ncclMRCRegMrDmaBufInternalPerDev(ncclIbNetCommDevBase* base, void* data, size_t size, int type, uint64_t offset, int fd, uint64_t mrFlags, struct ibv_mr** mhandle) {
...
/* DMA-BUF support */
ncclResult_t ncclMRCRegMrDmaBufInternal(void* comm, void* data, size_t size, int type, uint64_t offset, int fd, uint64_t mrFlags, void** mhandle) {
  ncclResult_t ret = ncclSuccess;
  assert(size > 0);
  struct ncclIbNetCommBase* base = (struct ncclIbNetCommBase*) comm;
  struct ncclIbMrHandle* mhandleWrapper = (struct ncclIbMrHandle*) calloc(1, sizeof(struct ncclIbMrHandle));
  if (!mhandleWrapper) return ncclInternalError;
  for (int i = 0; i < base->vProps.ndevs; i++) {
    ...
    NCCLCHECKGOTO(ncclMRCRegMrDmaBufInternalPerDev(devComm, data, size, type, offset, fd, mrFlags, mhandleWrapper->mrs + i), ret, fail);
  }
...
ncclResult_t ncclMRCRegMrDmaBuf(void* comm, void* data, size_t size, int type, uint64_t offset, int fd, void** mhandle) {
  return ncclMRCRegMrDmaBufInternal(comm, data, size, type, offset, fd, 0ULL, mhandle);
}

ncclResult_t ncclMRCRegMr(void* comm, void* data, size_t size, int type, void** mhandle) {
  return ncclMRCRegMrDmaBufInternal(comm, data, size, type, 0ULL, -1, 0, mhandle);
}

ncclResult_t ncclMRCDeregMRInternal(ncclIbNetCommDevBase* base, struct ibv_mr* mhandle) {
  struct ncclIbMrCache* cache = &ncclIbDevs[base->ibDevN].mrCache;
  ncclResult_t res;
  pthread_mutex_lock(&ncclIbDevs[base->ibDevN].lock);
...
ncclResult_t ncclMRCDeregMR(void* comm, void* mhandle) {
  ...
  for (int i = 0; i < base->vProps.ndevs; i++) {
    // Each ncclIbNetCommDevBase is at different offset in send and recv netComms
    struct ncclIbNetCommDevBase* devComm = ncclIbGetNetCommDevBase(base, i);
    NCCLCHECK(ncclMRCDeregMRInternal(devComm, mhandleWrapper->mrs[i]));
  }
  free(mhandleWrapper);
  return ncclSuccess;
}

ncclResult_t ncclMRCIsend(void* sendComm, void* data, size_t size, int tag, void* mhandle, void* phandle, void** request) {
  struct ncclIbSendComm* comm = (struct ncclIbSendComm*)sendComm;
  if (comm->base.ready == 0) {
    WARN("NET/IB: ncclMRCIsend() called when comm->base.ready == 0");
    *request = NULL;
    return ncclInternalError;
  }
  ...
  for (int r=0; r<nreqs; r++) {
    if (reqs[r] != NULL || slots[r].tag != (uint32_t)tag) continue;
...
ncclResult_t ncclMRCIrecv(void* recvComm, int n, void** data, size_t* sizes, int* tags, void** mhandles, void** phandles, void** request) {
  struct ncclIbRecvComm* comm = (struct ncclIbRecvComm*)recvComm;
  if (comm->base.ready == 0) {
    WARN("NET/IB: ncclMRCIrecv() called when comm->base.ready == 0");
    *request = NULL;
    return ncclInternalError;
  }
  NCCLCHECK(ncclIbStatsCheckFatalCount(&comm->base.stats,__func__));
  ...
  for (int i = 0; i < nqps; i++) {
    ...
    NCCLCHECK(wrap_mrc_post_recv(qp->mrcQp, &wr, &bad_wr));
    comm->base.qpIndex = (comm->base.qpIndex+1)%comm->base.nqps;
  }
  TIME_STOP(1);
...
ncclResult_t ncclMRCIflush(void* recvComm, int n, void** data, int* sizes, void** mhandles, void** request) {
...
ncclResult_t ncclMRCTest(void* request, int* done, int* sizes) {
  ...
  while (1) {
    ...
    if (r->events[0] == 0 && r->events[1] == 0 && r->events[2] == 0 && r->events[3] == 0) {
      ...
      *done = 1;
      ...
      else if (sizes && r->type == NCCL_NET_IB_REQ_SEND) {
        sizes[0] = r->send.size;
      }
    ...
    for (int i = 0; i < NCCL_IB_MAX_DEVS_PER_NIC; i++) {
      ...
      if (r->events[i]) {
        // FLUSH requests use the ibverbs CQ (base->cq); all others use the MRC CQ
...
ncclResult_t ncclMRCCloseSend(void* sendComm) {
  ...
  if (comm) {
    ...
    for (int q = 0; q < comm->base.nqps; q++) {
      if (comm->base.qps[q].mrcQp != NULL) NCCLCHECK(wrap_mrc_destroy_qp(comm->base.qps[q].mrcQp));

      // Then destroy the hint after QP is gone
      if (comm->base.qpHints[q] != NULL) {
        NCCLCHECK(wrap_mrc_destroy_qp_hint(comm->base.qpHints[q]));
        comm->base.qpHints[q] = NULL;
      }
    }
    for (int i = 0; i < comm->base.vProps.ndevs; i++) {
...
ncclResult_t ncclMRCCloseRecv(void* recvComm) {
  ...
  if (comm) {
    ...
    for (int i = 0; i < comm->base.vProps.ndevs; i++) {
      ...
      // Destroy flush QPs before their CQ (base->cq) is freed in ncclIbDestroyBase
...
ncclResult_t ncclMRCCloseListen(void* listenComm) {
  struct ncclMRCListenComm* comm = (struct ncclMRCListenComm*)listenComm;
  if (comm) {
...
// ---- Compatibility wrappers for older NCCL net ABI versions ----

ncclResult_t ncclMRCGetProperties_v10(int dev, ncclNetProperties_v10_t* props_v10) {
  ncclNetProperties_t props;
  ncclResult_t ret = ncclMRCGetProperties(dev, &props);
  if (ret != ncclSuccess) return ret;
  ...
  for (int i = 0; i < props.vProps.ndevs; i++) props_v10->vProps.devs[i] = props.vProps.devs[i];
  return ncclSuccess;
}

ncclResult_t ncclMRCGetProperties_v9(int dev, ncclNetProperties_v9_t* props_v9) {
  ncclNetProperties_t props;
  ncclResult_t ret = ncclMRCGetProperties(dev, &props);
  if (ret != ncclSuccess) return ret;
  ...
  for (int i = 0; i < props.vProps.ndevs; i++) props_v9->vProps.devs[i] = props.vProps.devs[i];
  return ncclSuccess;
}

ncclResult_t ncclMRCGetProperties_v8(int dev, ncclNetProperties_v8_t* props_v8) {
  ncclNetProperties_t props;
  ncclResult_t ret = ncclMRCGetProperties(dev, &props);
  if (ret != ncclSuccess) return ret;
...
ncclResult_t ncclMRCGetProperties_v7(int dev, ncclNetProperties_v7_t* props_v7) {
  ncclNetProperties_t props;
  ncclResult_t ret = ncclMRCGetProperties(dev, &props);
  if (ret != ncclSuccess) return ret;
...
ncclResult_t ncclMRCGetProperties_v6(int dev, ncclNetProperties_v6_t* props_v6) {
  ncclNetProperties_t props;
  ncclResult_t ret = ncclMRCGetProperties(dev, &props);
  if (ret != ncclSuccess) return ret;
...
ncclResult_t ncclMRCInit_v10(ncclDebugLogger_t logFunction, ncclProfilerCallback_t profilerCallback) {
  (void)profilerCallback;
  return ncclMRCInit(NULL, 0, NULL, logFunction, NULL);
}

ncclResult_t ncclMRCConnect_v10(int dev, ncclNetCommConfig_v10_t* config, void* opaqueHandle, void** sendComm, ncclNetDeviceHandle_v10_t** sendDevComm) {
  (void)config;
  return ncclMRCConnect(NULL, dev, opaqueHandle, sendComm, (ncclNetDeviceHandle_t**)sendDevComm);
}

ncclResult_t ncclMRCListen_v10(int dev, void* opaqueHandle, void** listenComm) {
  return ncclMRCListen(NULL, dev, opaqueHandle, listenComm);
}

ncclResult_t ncclMRCConnect_v9(int dev, void* opaqueHandle, void** sendComm, ncclNetDeviceHandle_t** sendDevComm) {
  return ncclMRCConnect_v10(dev, NULL, opaqueHandle, sendComm, (ncclNetDeviceHandle_v10_t**)sendDevComm);
}

ncclResult_t ncclMRCInit_v9(ncclDebugLogger_t logFunction) {
  return ncclMRCInit_v10(logFunction, NULL);
}

ncclResult_t ncclMRCIsend_v9(void* sendComm, void* data, size_t size, int tag, void* mhandle, void** request) {
  return ncclMRCIsend(sendComm, data, size, tag, mhandle, NULL, request);
}

ncclResult_t ncclMRCIsend_v8(void* sendComm, void* data, int size, int tag, void* mhandle, void** request) {
  return ncclMRCIsend_v9(sendComm, data, (size_t)size, tag, mhandle, request);
}

ncclResult_t ncclMRCIrecv_v9(void* recvComm, int n, void** data, size_t* sizes, int* tags, void** mhandles, void** request) {
  return ncclMRCIrecv(recvComm, n, data, sizes, tags, mhandles, NULL, request);
}

ncclResult_t ncclMRCIrecv_v8(void* recvComm, int n, void** data, int* sizes, int* tags, void** mhandles, void** request) {
  size_t sizesOut[NCCL_NET_IB_MAX_RECVS];
  for (int i = 0; i < n; i++) sizesOut[i] = sizes[i];
  return ncclMRCIrecv_v9(recvComm, 1, data, sizesOut, tags, mhandles, request);
}

ncclResult_t ncclMRCRegMr_v7(void* comm, void* data, int size, int type, void** mhandle) {
  return ncclMRCRegMrDmaBuf(comm, data, (size_t)size, type, 0ULL, -1, mhandle);
}

ncclResult_t ncclMRCConnect_v6(int dev, void* opaqueHandle, void** sendComm) {
  ncclNetDeviceHandle_v7_t* handle = NULL;
  return ncclMRCConnect_v9(dev, opaqueHandle, sendComm, &handle);
}

ncclResult_t ncclMRCAccept_v6(void* listenComm, void** recvComm) {
  ncclNetDeviceHandle_v7_t* handle = NULL;
  return ncclMRCAccept(listenComm, recvComm, &handle);
}

const ncclNet_v11_t ibPlugin_v11 = {
  .name = "mrc_v11",
  .init = ncclMRCInit,
  .devices = ncclMRCDevices,
  .getProperties = ncclMRCGetProperties,
  .listen = ncclMRCListen,
  .connect = ncclMRCConnect,
  .accept = ncclMRCAccept,
  .regMr = ncclMRCRegMr,
  .regMrDmaBuf = ncclMRCRegMrDmaBuf,
  .deregMr = ncclMRCDeregMR,
  .isend = ncclMRCIsend,
  .irecv = ncclMRCIrecv,
  .iflush = ncclMRCIflush,
  .test = ncclMRCTest,
  .closeSend = ncclMRCCloseSend,
  .closeRecv = ncclMRCCloseRecv,
  .closeListen = ncclMRCCloseListen,
  NULL /* getDeviceMr */,
  NULL /* irecvConsumed */,
  ncclIbMakeVDevice,
  ncclIbFinalize,
...
const ncclNet_v10_t ibPlugin_v10 = {
  .name = "mrc_v10",
  .init = ncclMRCInit_v10,
  .devices = ncclMRCDevices,
  .getProperties = ncclMRCGetProperties_v10,
  .listen = ncclMRCListen_v10,
  .connect = ncclMRCConnect_v10,
  .accept = ncclMRCAccept,
  .regMr = ncclMRCRegMr,
  .regMrDmaBuf = ncclMRCRegMrDmaBuf,
  .deregMr = ncclMRCDeregMR,
...
const ncclNet_v9_t ibPlugin_v9 = {
  ...
  .init = ncclMRCInit_v9,
  .devices = ncclMRCDevices,
  .getProperties = ncclMRCGetProperties_v9,
  .listen = ncclMRCListen_v10,
  .connect = ncclMRCConnect_v9,
  .accept = ncclMRCAccept,
  .regMr = ncclMRCRegMr,
  .regMrDmaBuf = ncclMRCRegMrDmaBuf,
  .deregMr = ncclMRCDeregMR,
  .isend = ncclMRCIsend_v9,
  .irecv = ncclMRCIrecv_v9,
  .iflush = ncclMRCIflush,
  .test = ncclMRCTest,
  .closeSend = ncclMRCCloseSend,
  .closeRecv = ncclMRCCloseRecv,
...
const ncclNet_v8_t ibPlugin_v8 = {
  ...
  .init = ncclMRCInit_v9,
  .devices = ncclMRCDevices,
  .getProperties = ncclMRCGetProperties_v8,
  .listen = ncclMRCListen_v10,
  .connect = ncclMRCConnect_v9,
  .accept = ncclMRCAccept,
  .regMr = ncclMRCRegMr,
  .regMrDmaBuf = ncclMRCRegMrDmaBuf,
  .deregMr = ncclMRCDeregMR,
  .isend = ncclMRCIsend_v8,
  .irecv = ncclMRCIrecv_v8,
  .iflush = ncclMRCIflush,
  .test = ncclMRCTest,
  .closeSend = ncclMRCCloseSend,
  .closeRecv = ncclMRCCloseRecv,
...
const ncclNet_v7_t ibPlugin_v7 = {
  ...
  .init = ncclMRCInit_v9,
  .devices = ncclMRCDevices,
  .getProperties = ncclMRCGetProperties_v7,
  .listen = ncclMRCListen_v10,
  .connect = ncclMRCConnect_v9,
  .accept = ncclMRCAccept,
  .regMr = ncclMRCRegMr_v7,
  .regMrDmaBuf = ncclMRCRegMrDmaBuf,
  .deregMr = ncclMRCDeregMR,
  .isend = ncclMRCIsend_v8,
  .irecv = ncclMRCIrecv_v8,
...
const ncclNet_v6_t ibPlugin_v6 = {
  ...
  .init = ncclMRCInit_v9,
  .devices = ncclMRCDevices,
  .getProperties = ncclMRCGetProperties_v6,
  .listen = ncclMRCListen_v10,
  .connect = ncclMRCConnect_v6,
  .accept = ncclMRCAccept_v6,
  .regMr = ncclMRCRegMr_v7,
  .regMrDmaBuf = ncclMRCRegMrDmaBuf,
  .deregMr = ncclMRCDeregMR,
  .isend = ncclMRCIsend_v8,
